...
def test_reward_components():
    ...
    print(f"  Diversity rewards: {np.mean(diversity_rewards):.3f} ± {np.std(diversity_rewards):.3f}")
    print(f"  Novelty rewards: {np.mean(novelty_rewards):.3f} ± {np.std(novelty_rewards):.3f}")
    print(f"  Cost penalties: {np.mean(cost_penalties):.3f} ± {np.std(cost_penalties):.3f}")
    print(f"  Activity rewards: {np.mean(activity_rewards):.3f} ± {np.std(activity_rewards):.3f}")
...
def test_diversity_constraints():
    ...
    for step in range(5):
        ...
        if satisfies_constraint:
            ...
            print(f"  Action rejected due to diversity constraint")
...
def main():
    ...
    if passed == total:
        print(f"🎉 All tests passed! Enhanced environment is working correctly.")
    else:
