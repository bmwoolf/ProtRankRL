...
def analyze_protein_selections(
    ...
    analysis = {
        "total_selections": len(all_selections) * len(all_selections[0]) if all_selections else 0,
        "unique_proteins_selected": len(selection_frequencies),
        "selection_frequencies": selection_frequencies,
        "most_frequently_selected": sorted(selection_frequencies.items(), key=lambda x: x[1], reverse=True)[:10],
        "average_reward_per_selection": np.mean([s["reward"] for episode in all_selections for s in episode]),
        "hit_rate": np.mean([s["was_hit"] for episode in all_selections for s in episode]),
        "average_diversity_score": np.mean([s["diversity_score"] for episode in all_selections for s in episode]),
        "average_novelty_score": np.mean([s["novelty_score"] for episode in all_selections for s in episode]),
        "average_cost_penalty": np.mean([s["cost_penalty"] for episode in all_selections for s in episode]),
    }
...
def print_evaluation_summary(comparison_results: Dict[str, Any]) -> None:
    ...
    print(f"\n{'='*80}")
    print(f"ENHANCED RL AGENT EVALUATION SUMMARY")
    print(f"{'='*80}")
    
    # RL Agent Performance
    print(f"\n🤖 RL AGENT PERFORMANCE:")
    rl_metrics = comparison_results["rl_agent"]
    print(f"  Total Reward: {rl_metrics.get('mean_total_reward', 0):.3f} ± {rl_metrics.get('std_total_reward', 0):.3f}")
    print(f"  Hit Rate: {rl_metrics.get('mean_hit_rate', 0):.3f} ± {rl_metrics.get('std_hit_rate', 0):.3f}")
    print(f"  Average Diversity: {rl_metrics.get('mean_avg_diversity', 0):.3f} ± {rl_metrics.get('std_avg_diversity', 0):.3f}")
    print(f"  Average Novelty: {rl_metrics.get('mean_avg_novelty', 0):.3f} ± {rl_metrics.get('std_avg_novelty', 0):.3f}")
    print(f"  Activity Strength: {rl_metrics.get('mean_avg_activity_strength', 0):.3f} ± {rl_metrics.get('std_avg_activity_strength', 0):.3f}")
...
Examples:
  ...
  python examples/evaluate_enhanced_agent.py --model-path models/enhanced_ppo_final --comprehensive
    ...
    if args.comprehensive:
        ...
        print(f"Protein Selection Analysis:")
        ...
        print(f"  Average diversity: {selection_analysis['average_diversity_score']:.3f}")
    ...
    print(f"  - logs/method_comparison.json")
    if args.comprehensive:
        print(f"  - logs/protein_selection_analysis.json")
    if args.save_plots:
        print(f"  - logs/evaluation_results.png")
