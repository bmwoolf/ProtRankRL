...
class UnifiedProteinDataLoader:
    ...
    def load_unified_dataset(self, file_path: Optional[str] = None) -> pd.DataFrame:
        """
        ...
        logger.info(f"Loaded unified dataset with {len(data)} proteins from {file_path}")
        return data
    ...
    def prepare_environment_data(
        ...
        if use_activity_features:
            ...
            available_activity_cols = [col for col in activity_cols if col in data_df.columns]
        ...
        logger.info(f"Prepared {len(features)} proteins with {features.shape[1]} features")
    ...
    def get_data_summary(self, data_df: pd.DataFrame) -> Dict[str, Union[int, float]]:
        ...
        if 'activity_count' in data_df.columns:
            summary['mean_activities_per_protein'] = float(data_df['activity_count'].mean())
            summary['median_activities_per_protein'] = float(data_df['activity_count'].median())
...
def load_unified_protein_data(
    ...
    features, targets = loader.prepare_environment_data(
        data_df, target_column, use_embeddings, use_activity_features, normalize_features
    )
