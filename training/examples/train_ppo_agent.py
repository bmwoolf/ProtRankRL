...
def make_env():
    ...
    def _make_env():
        return create_synthetic_env(
            num_proteins=32, feature_dim=64, hit_rate=0.2, seed=None
        )
...
def train_and_evaluate(timesteps, model_name):
    ...
    test_env = create_synthetic_env(
        num_proteins=32, feature_dim=64, hit_rate=0.2, seed=123
    )
    ...
    print(
        f"  Training Time: {training_time:.1f} seconds ({training_time/60:.1f} minutes)"
    )
...
def main():
    ...
    print(f"{'='*60}")
    print(
        f"{'Timesteps':<12} {'Time(s)':<8} {'Time(m)':<8} {'Reward':<8} {'Hits':<6} {'Hit Rate':<10}"
    )
    print("-" * 60)

    for result in results:
        print(
            f"{result['timesteps']:<12,} {result['training_time']:<8.1f} "
