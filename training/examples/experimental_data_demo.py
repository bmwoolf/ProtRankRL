...
def run_experimental_demo():
    ...
    try:
        ...
        console.print(
            f"  Hits: {np.sum(experimental_env.targets)}/{experimental_env.num_proteins}"
        )
        ...
        if protein_details:
            ...
            for protein in protein_details[:10]:  # Show first 10
                table.add_row(
                    ...
                    (
                        protein["pref_name"][:15] + "..."
                        if len(protein["pref_name"]) > 15
                        else protein["pref_name"]
                    ),
        ...
        console.print(
            "[red]No ChEMBL data found. Run scripts/download_chembl_data.py first.[/red]"
        )
        ...
        try:
            ...
            console.print(
                f"  Hits: {np.sum(legacy_env.targets)}/{legacy_env.num_proteins}"
            )
        ...
        for reward_type in reward_types:
            ...
            try:
                env = create_experimental_env(
                    data_source="chembl", reward_type=reward_type, max_proteins=10
                )
